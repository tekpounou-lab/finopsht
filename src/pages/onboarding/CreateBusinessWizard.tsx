...
export default function CreateBusinessWizard({
  ...
  const handleFinish = () => {
    ...
    // Construct departments with unique IDs
    ...
    const createdEmployees: Employee[] = [
      // Create owner employee profile too
      {
        ...
        departmentId: deptObjects[0]?.id || "d1",
        department_id: deptObjects[0]?.id || "d1",
        name: userName,
        email: userEmail,
        role: "OWNER",
        baseSalary: 120000,
      ...
      ...teamInvites.map((inv, idx) => ({
        ...
        branchId: newBranchId,
        branch_id: newBranchId,
        departmentId: deptObjects[1]?.id || deptObjects[0]?.id || "d1",
        department_id: deptObjects[1]?.id || deptObjects[0]?.id || "d1",
        name: inv.name,
        email: inv.email,
        role: inv.role,
        baseSalary: inv.baseSalary,
      ...
      <div className="flex justify-between items-center border-t border-slate-900 pt-5 mt-8" id="wizard-navigation-footer">
        ...
        {step < 5 ? (
          <button
            ...
            className="px-5 py-2 bg-cyan-600 hover:bg-cyan-550 text-slate-950 rounded-lg text-xs font-black font-mono uppercase tracking-wide flex items-center gap-1.5 cursor-pointer transition shadow-md shadow-cyan-500/10"
          >
          ...
          <button
            ...
            className="px-6 py-2 bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-400 hover:to-teal-500 text-slate-950 rounded-lg text-xs font-black font-mono uppercase tracking-wide flex items-center gap-1.5 cursor-pointer transition shadow-lg shadow-emerald-500/20"
          >
