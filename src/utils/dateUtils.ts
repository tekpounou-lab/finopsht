 ...
 */
export function normalizeCsvDate(rawDateStr: any, fallbackDate?: string): string {
  ...
  if (parts.length === 3) {
    ...
    if (!isNaN(p1) && !isNaN(p2) && !isNaN(p3)) {
      ...
      // Default to US format MM/DD/YYYY (M/D/YYYY) which is standard for CSV templates & QuickBooks/Excel exports
      else {
        month = p1;
        day = p2;
      }
